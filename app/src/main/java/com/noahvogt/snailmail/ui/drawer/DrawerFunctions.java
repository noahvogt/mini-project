...
public class DrawerFunctions {
    ...
    public void changeMailServerSettingsDialog(String name, String email, String password,
            View headerView, String imapHost, String smtpHost, int imapPort, int smtpPort,
            boolean wantToAddNew, LayoutInflater layoutInflater, Context context,
            SharedPreferences mailServerCredentials) {
        /* define View window */
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(context);
        final View changeMailServerSettingsView = layoutInflater
                .inflate(R.layout.mail_credentials_customizer, null);

        /* access objects */
        EditText incomingServerObject = (EditText) changeMailServerSettingsView
                .findViewById(R.id.custom_mail_server_incoming_server_text);
        EditText outgoingServerObject = (EditText) changeMailServerSettingsView
                .findViewById(R.id.custom_mail_server_outgoing_server_text);
        EditText incomingPortObject = (EditText) changeMailServerSettingsView
                .findViewById(R.id.custom_mail_server_incoming_port_text);
        EditText outgoingPortObject = (EditText) changeMailServerSettingsView
                .findViewById(R.id.custom_mail_server_outgoing_port_text);
        EditText serverUsernameObject = (EditText) changeMailServerSettingsView
                .findViewById(R.id.custom_mail_server_username_text);
        EditText passwordObject = (EditText) changeMailServerSettingsView
                .findViewById(R.id.custom_mail_server_password_text);
        Button saveButton = (Button) changeMailServerSettingsView
                .findViewById(R.id.saveCustomizeButton);
        Button cancelButton = (Button) changeMailServerSettingsView
                .findViewById(R.id.cancelCustomizeButton);
        ...
        saveButton.setOnClickListener(view -> {
            if (wantToAddNew) {
                addNewAccountCredentials(name, serverUsernameObject.getText().toString(),
                        passwordObject.getText().toString(),
                        Integer.parseInt(incomingPortObject.getText().toString()),
                        Integer.parseInt(outgoingPortObject.getText().toString()),
                        incomingServerObject.getText().toString(),
                        outgoingServerObject.getText().toString(), rootChangeServerSettingsDialog,
                        false, headerView, mailServerCredentials, context, layoutInflater);
            } else {
    ...
    public boolean addNewAccountCredentials(String name, String email, String password,
            int imapPort, int smtpPort, String imapHost, String smtpHost,
        ...
        if (ConnectionBackgroundChecker.canConnect(imapHost, imapPort, email, password)) {
            ...
            SharedPreferences initialCredentialsReader = context.getSharedPreferences("Credentials",
            ...
            ArrayList<MailServerCredentials> allUsersCredentials = gson
                    .fromJson(initialReadJsonData, credentialsType);
            ...
            try {
                ...
                System.out.println(
                        "creating new arraylist for user credentials, as it seems to be empty");
                allUsersCredentials = new ArrayList<>();
            }

            /* add new email account if the email hasn't been entered before */
            if (newEmailIsUnique) {
                allUsersCredentials.add(new MailServerCredentials(name, email, password, imapHost,
                        smtpHost, imapPort, smtpPort, ""));
                credentialsEditor.putString("data",
                        gson.toJson(allUsersCredentials, credentialsType));
                ...
                Toast.makeText(context, "You cannot add the same email twice", Toast.LENGTH_SHORT)
                        .show();
                return false;
            }
        } else {
            if (wantConnectionFailedDialog) {
                askForChangeMailServerSettingsDialog(name, email, password, headerView,
                        layoutInflater, context, mailServerCredentials);
            } else {
    ...
    public void askForChangeMailServerSettingsDialog(String name, String email, String password,
            View headerView, LayoutInflater layoutInflater, Context context,
            SharedPreferences mailServerCredentials) {
        /* define View window */
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(context);
        ...
        dialogBuilder.setMessage("Do you want to further customize your mail server settings?")
                .setPositiveButton("Yes", (dialog, id) -> {
                    /* if this button is clicked, close the whole fragment */
                    changeMailServerSettingsDialog(name, email, password, headerView,
                            MailFunctions.getImapHostFromEmail(email),
                            MailFunctions.getSmtpHostFromEmail(email), 993, 587, true,
                            layoutInflater, context, mailServerCredentials);
    ...
    public void updateNavHeaderText(View headerView, Context context) {
        SharedPreferences mailServerCredentials = context.getSharedPreferences("Credentials",
                Context.MODE_PRIVATE);
        ...
        if (!startupUser.isEmpty()) {
            ...
            ArrayList<MailServerCredentials> startupUsersCredentials = gson
                    .fromJson(startupJsonData, credentialsType);
    ...
    /*
     * use 'initialMail' variable so that the program knows which email entry is has
     * to change
     */
    public void changeAccountCredentials(String name, String email, String password, int imapPort,
            ...
            View headerView, String initialEmail, SharedPreferences mailServerCredentials,
            Context context) {
        /* ugly fix, please further investigate */
        if (mailServerCredentials == null) {
            SharedPreferences whenNullPointerCredentials = context
        ...
        if (ConnectionBackgroundChecker.canConnect(imapHost, imapPort, email, password)) {
            ...
            for (int i = 0; i < allUsersCredentials.size(); i++) {
                if (allUsersCredentials.get(i).getUsername().equals(initialEmail)) {
                    ...
                    allUsersCredentials.set(i, new MailServerCredentials(name, email, password,
                            imapHost, smtpHost, imapPort, smtpPort, signature));
                    credentialsEditor
                            .putString("data", gson.toJson(allUsersCredentials, credentialsType))
                            .apply();
                    dialogContext.dismiss();
                    break;
                }
    ...
    public void createNewEmailDialog(View headerView, LayoutInflater layoutInflater,
            Context context) {
        /* define View window */
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(context);
        ...
        EditText newEmailAddress = emailPopupView
                .findViewById(R.id.popup_material_email_asking_text);
        EditText newEmailPassword = emailPopupView
                .findViewById(R.id.popup_material_password_asking_text);
        ...
        newEmailSaveButton.setOnClickListener(v -> {
            ...
            if (!MailFunctions.validateEmail(newEmailAddress)
                    | !MailFunctions.validateName(newEmailName)
                    | !MailFunctions.validatePassword(newEmailPassword)) {
                return;
            }

            boolean connection = addNewAccountCredentials(name, email, password, 993, 587,
                    MailFunctions.getImapHostFromEmail(email),
                    MailFunctions.getSmtpHostFromEmail(email), rootCreateNewEmailPopupDialog, true,
    ...
    public void accountManagerViewHandler(Context context, LayoutInflater layoutInflater,
            View headerView, SharedPreferences mailServerCredentials) {
        /* define dialog */
        AlertDialog.Builder dialogBuilder = new AlertDialog.Builder(context);
        final View accountManagerView = layoutInflater.inflate(R.layout.account_manager, null);

        AutoCompleteTextView accountSelectorObject = (AutoCompleteTextView) accountManagerView
                .findViewById(R.id.accountSelectorTextView);

        /* get string data for drop down menu */
        SharedPreferences credReader = context.getSharedPreferences("Credentials",
                Context.MODE_PRIVATE);
        String currentUser = credReader.getString("currentUser", "");

        TextView showCurrentUserObject = (TextView) accountManagerView
                .findViewById(R.id.showCurrentUser);
        Button switchAccountObject = (Button) accountManagerView
                .findViewById(R.id.switchToAccountButton);
        Button deleteAccountObject = (Button) accountManagerView
                .findViewById(R.id.deleteAccountButton);
        Button changeServerSettingsObject = (Button) accountManagerView
                .findViewById(R.id.changeServerSettingsButton);
        ...
        ArrayList<MailServerCredentials> currentUsersCredentials = gson.fromJson(jsonCredData,
                credentialsType);
        ...
        switchAccountObject.setOnClickListener(view -> {
            ...
            Type credentialsType1 = new TypeToken<ArrayList<MailServerCredentials>>() {
            }.getType();
            ArrayList<MailServerCredentials> currentUsersCredentials1 = gson.fromJson(jsonCredData1,
                    credentialsType1);
                ...
                for (int i = 0; i < currentUsersCredentials1.size(); i++) {
                    if (currentUsersCredentials1.get(i).getUsername().equals(userInput)) {
                        ...
                        showCurrentUserObject
                                .setText(String.format("current user:\n%s", userInput));
        ...
        deleteAccountObject.setOnClickListener(view -> {
            ...
            ArrayList<MailServerCredentials> currentUserCredentials = gson.fromJson(jsonCredData12,
                    credentialsType12);
            ...
            try {
                for (int i = 0; i < currentUserCredentials.size(); i++) {
                    if (currentUserCredentials.get(i).getUsername().equals(userInput)) {
                        ...
                        ArrayAdapter<String> newDropDownAdapter = new ArrayAdapter<>(
                                context.getApplicationContext(), R.layout.dropdown_item,
                        ...
                        credEditor
                                .putString("data",
                                        gson.toJson(currentUserCredentials, credentialsType12))
                                .apply();
        ...
        changeServerSettingsObject.setOnClickListener(view -> {
            ...
            Type credentialsType13 = new TypeToken<ArrayList<MailServerCredentials>>() {
            }.getType();
            ArrayList<MailServerCredentials> currentUserCredentials = gson.fromJson(jsonCredData13,
                    credentialsType13);
            ...
            try {
                for (int i = 0; i < currentUserCredentials.size(); i++) {
                    if (currentUserCredentials.get(i).getUsername().equals(userInput)) {
                        ...
                        changeMailServerSettingsDialog(name, email, password, headerView, imapHost,
                                smtpHost, imapPort, smtpPort, false, layoutInflater, context,
    ...
    public void newUserAlphaSoftwareWarningBeforeAddingEmail(boolean button, View view,
    ...
    public boolean handleMasterDotsMenu(MenuItem menuItem, Context context,
            LayoutInflater layoutInflater) {
